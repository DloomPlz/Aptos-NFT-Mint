...
import {
  ...
  NODE_URL,
  CONTRACT_ADDRESS,
} from './candyMachineInfo'

async function getCandyMachineResourceAccount() {
  const response = await axios.get(
    `${NODE_URL}/accounts/${candyMachineAddress}/resources`
  )
  const resources = response.data

  for (const resource of resources) {
    if (
      resource.type === `${CONTRACT_ADDRESS}::candy_machine_v2::ResourceData`
    ) {
  ...
  console.error(
    `Candy machine not initialized on given address for chain ${mode}`
  )
  return null
}

async function getCandyMachineCollectionInfo(cmResourceAccount) {
  const response = await axios.get(
    `${NODE_URL}/accounts/${cmResourceAccount}/resources`
  )
  ...
  for (const resource of cmResourceAccountResources) {
    if (resource.type === '0x3::token::Collections') {
      collectionInfo.numUploadedTokens =
        resource.data.create_token_data_events.counter
    ...
    if (
      resource.type ===
      `${CONTRACT_ADDRESS}::candy_machine_v2::CollectionConfigs`
    ) {
      collectionInfo.candyMachineConfigHandle =
        resource.data.collection_configs.handle
    }
  }
...
async function getCandyMachineConfigData(candyMachineConfigHandle) {
  ...
  const response = await axios.post(
    `${NODE_URL}/tables/${candyMachineConfigHandle}/item`,
    data,
    customConfig
  )
...
async function getMintedNfts(
  ...
  for (const event of txInfo.events) {
    ...
    try {
      mintedNft.imageUri = (
        await aptosClient.getTableItem(collectionTokenDataHandle, {
